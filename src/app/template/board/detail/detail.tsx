...
import { BoardDetailComment, BoardDetailCommentDate, BoardDetailCommentText, BoardDetailCommentTop, BoardDetailCommentWriter, BoardDetailContent, BoardDetailContentBottom, BoardDetailContentCategory, BoardDetailContentCommentCount, BoardDetailContentDate, BoardDetailContentText, BoardDetailContentTitle, BoardDetailContentTop, BoardDetailContentTopWrap, BoardDetailContentWriter, BoardDetailPostComment, BoardDetailPostCommentButton, BoardDetailPostCommentButtonText, BoardDetailPostCommentForm, BoardDetailPostCommentInput} from "./styled";
        ...
        if (id) {
          const fetchPost = async () => {
            try {
              ...
              setIsError("게시글을 불러오는 중 오류가 발생했습니다.");
            } finally {
              setIsLoading(false);
            }
          };
        ...
        try {
          const response = await fetch("/api/post-comments", {
            ...
            body: JSON.stringify({
              ...
              postId: id, // 게시글 ID
              userId: currentUser?.username, // 로그인한 사용자 ID를 넣어야 함 (지금은 임시로 1)
            }),
          });
    ...
    <MainContainer>
      ...
      <BoardDetailPostComment>
        <BoardDetailPostCommentForm onSubmit={handleSubmit(onSubmit)}>
            <BoardDetailPostCommentInput {...register("content", { required: true })} placeholder="댓글을 입력하세요" />
